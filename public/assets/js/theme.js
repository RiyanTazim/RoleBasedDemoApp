...
function layout_change_default() {
  // Force default layout to dark
  let dark_layout = 'dark';
  ...
  // Optional: Remove system preference change listener
  // window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', (event) => {
  //   dark_layout = event.matches ? 'dark' : 'light';
  //   layout_change(dark_layout);
  // });
...
function layout_change(layout) {
  ...
  var logoSrc = isDark ? '../assets/images/softvence.jpg' : '../assets/images/softvence.jpg';
...
function layout_theme_sidebar_change(value) {
  if (value == 'true') {
    ...
    if (document.querySelector('.pc-sidebar .m-header .logo-lg')) {
      document.querySelector('.pc-sidebar .m-header .logo-lg').setAttribute('src', '../assets/images/softvence.jpg');
    }
    var control = document.querySelector('.theme-nav-layout .btn.active');
    if (control) {
      document.querySelector('.theme-nav-layout .btn.active').classList.remove('active');
